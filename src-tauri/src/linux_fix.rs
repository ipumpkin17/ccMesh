...
//! single_instance 回调、托盘 show_main）都应在现有 `set_focus()` 之后
//! 追加一次调用。
//!
//! 移植自 cc-switch（`src-tauri/src/linux_fix.rs`），日志改用本项目的
//! `tracing`，并将函数泛型化以兼容托盘侧的泛型 `Runtime`。
...
///
/// 调用是 fire-and-forget：内部 spawn 一个异步任务在 ~800ms 后完成。
/// 调用线程立即返回，不阻塞 UI。
        ...
        match window.inner_size() {
            Ok(original) => {
                ...
                match window.inner_size() {
                    ...
                    Err(e) => {
                        tracing::warn!("Linux nudge: 对账回读 inner_size 失败: {e}");
                    }
                }
            }
            Err(e) => {
                ...
                tracing::warn!("Linux nudge: 读取 inner_size 失败，跳过伪 resize: {e}");
            }
        }
    });
}
