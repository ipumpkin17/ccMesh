...
//!
//! 严格只读：`OpenFlags::SQLITE_OPEN_READ_ONLY | SQLITE_OPEN_NO_MUTEX`，绝不写 cc-switch.db。
...
pub struct ProviderRow {
    ...
    pub app_type: String,
    pub name: String,
    pub settings_config: String, // JSON 字符串
    pub meta: String,            // JSON 字符串
    pub notes: Option<String>,
}

const APP_TYPES: &[&str] = &["claude", "codex"];

/// 只读打开 cc-switch.db 并返回 claude/codex 供应商行（按 sort_index, created_at, id 排序）。
