...
export function SettingsDataTable({
  ...
  columns: readonly SettingsDataTableColumn[];
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-edge-subtle">
            {columns.map((column, index) => (
              <th
                key={index}
                className={`px-4 py-2 ${column.align === "right" ? "text-right" : "text-left"} ${tableHeadClass}`}
              >
                {column.label}
              </th>
            ))}
        ...
        <tbody>
          {rows.map((row, rowIndex) => (
            <tr key={rowIndex} className="border-b border-edge-subtle last:border-0">
              {row.map((cell, columnIndex) => (
                <td
                  key={columnIndex}
                  className={`px-4 py-2 ${columns[columnIndex]?.align === "right" ? "text-right" : "text-left"}`}
                >
                  {cell}
                </td>
              ))}
...
export function SettingsTableAction({
  children,
  onClick,
  disabled,
}: {
