...
export const ccSwitchSourceApi: ExternalMigrationSourceApi = {
  preview: (dbPath) => request<PreviewItem[]>('preview_cc_switch_import', { dbPath }),
  import: (ids, dbPath) => request<ImportSummary>('import_cc_switch_providers', { ids, dbPath }),
}

/** CPA 源：默认路径或上传文件路径。 */
export const cpaSourceApi: ExternalMigrationSourceApi = {
  preview: (path) => request<PreviewItem[]>('preview_cpa_import', { path }),
  import: (ids, path) => request<ImportSummary>('import_cpa_providers', { ids, path }),
}
