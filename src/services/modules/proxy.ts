...
export const proxyApi = {
  ...
  switchEndpoint: (endpointId: string) => request<ProxyStatus>('switch_endpoint', { endpointId }),
  /** 订阅代理状态变更事件，返回取消订阅函数。 */
  onStatusChanged: (cb: (status: ProxyStatus) => void): Promise<UnlistenFn> => subscribe<ProxyStatus>(Events.proxyStatusChanged, (e) => cb(e.payload)),
}
