...
import { toolConfigApi, type ChannelMeta, type ClaudeOperationFields } from '@/services/modules/tool_config'
...
export function ClaudeWorkspace() {
  ...
  const syncOp = (f: ClaudeOperationFields) => setOpText(JSON.stringify(claudeOperationFragment(f), null, 2))
  ...
  const buildSnapshot = () => (rightEditable ? JSON.parse(rightText) : applyClaudeToggles(mergeClaudeSettings(base, fields), toggles))

  const saveCh = useMutation({
    mutationFn: async () => toolConfigApi.save('claude', { id: selectedId, name, snapshot: buildSnapshot() }),
    onSuccess: (meta) => {
  ...
  const setModel = (key: 'sonnetModel' | 'opusModel' | 'haikuModel', b: string, is1m: boolean) => updateFields({ [key]: withOneM(b, is1m) } as Partial<ClaudeOperationFields>)
  ...
  const togglePatterns = useMemo(() => {
    ...
    return (Object.keys(keyOf) as (keyof ClaudeToggles)[]).filter((k) => toggles[k]).map((k) => keyOf[k])
  }, [toggles])
      ...
      <Dialog open={!!pendingDelete} onOpenChange={(o) => !o && setPendingDelete(null)}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" disabled={delCh.isPending} onClick={() => pendingDelete && delCh.mutate(pendingDelete.id)}>
              删除
            </Button>
          </DialogFooter>
        </DialogContent>
