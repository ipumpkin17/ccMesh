...
import {
  DEFAULT_ENDPOINT_QUALITY_BUCKET_COUNT,
  useEndpointQuality,
} from "@/hooks/useEndpointQuality";
...
function blockTone(block: EndpointQualityBlock) {
  ...
  if (block.throttledCount > 0) return "bg-warning";
  return "bg-success";
}
...
function EndpointQualityTimeline({
  quality,
  bucketCount,
  variant,
}: {
  quality: EndpointQuality | undefined;
  bucketCount: number;
  variant: EndpointQualityVariant;
}) {
  const hasQuality = quality?.startedAtMs !== null && quality !== undefined;
  const blocks: Array<EndpointQualityBlock | null> = hasQuality
    ? quality.blocks
  ...
  return (
    <div className="endpoint-quality-strip flex w-max min-w-0 flex-col gap-1">
      <div
        className="grid h-4 grid-cols-3 items-center whitespace-nowrap text-[10px] leading-4 text-ink-secondary"
      ...
      >
        ...
        <span className="text-center">
          ...
          <TabularText className={quality && quality.successCount > 0 ? "text-success" : undefined}>
            {quality?.successCount ?? 0}
          </TabularText>
        </span>
        <span className="text-right">
          <span className="mr-0.5 text-ink-mute">失败</span>
          <TabularText className={quality && quality.failureCount > 0 ? "text-destructive" : undefined}>
      ...
      <div className="endpoint-quality-track flex items-center gap-2">
        ...
        >
          {blocks.map((block, index) => {
            if (block === null || !quality) {
              return (
                <Tooltip key={index}>
                  ...
                  <TooltipContent
                    sideOffset={6}
                    className="max-w-none whitespace-nowrap px-2 py-1 text-[11px] leading-4"
                  >
                    代理启动后开始记录
                  </TooltipContent>
                </Tooltip>
            ...
            const slotEndMs =
              index === blocks.length - 1 ? quality.windowEndMs : slotStartMs + quality.bucketMs;
            return (
              <Tooltip key={slotStartMs}>
                <TooltipTrigger asChild>
                  <span
                    className={cn(
                      "rounded-full transition-opacity [@media(hover:hover)]:hover:opacity-70",
                      blockTone(block),
                    )}
                  />
                </TooltipTrigger>
        ...
        <TabularText
          className={cn(
            ...
            quality?.successRate != null ? "text-success" : "text-ink-mute",
          )}
        >
