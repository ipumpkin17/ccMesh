...
function SortableRow({ endpoint, index, draggable, view, onEdit }: RowProps) {
  ...
  return (
    <div
      ref={ref}
      data-endpoint-row-id={endpoint.id}
...
function PreviewRow({ endpoint, index, visible, view }: PreviewRowProps) {
  ...
  return (
    ...
    >
      <div
        className={
          visible ? "pointer-events-none opacity-80" : "pointer-events-none opacity-60"
        }
      >
        <EndpointCard
          endpoint={endpoint}
  ...
  return (
    <DragDropProvider
      ...
      onDragEnd={(event) => {
        ...
        const next = showGlobalPreview && activeId !== null && typeof targetId === "number"
          ? moveBeforeEndpoint(order, activeId, targetId)
          : move(order, event);
        if (
          next.length === order.length &&
          next.every((item, index) => item.id === order[index]?.id)
        ) {
          return;
        }
