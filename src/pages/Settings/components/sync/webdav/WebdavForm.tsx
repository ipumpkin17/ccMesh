...
export function WebdavForm({ onSaved, layout = 'grid' }: { onSaved?: () => void; layout?: 'grid' | 'stack' }) {
  ...
  const set = (k: keyof WebDavConfig, v: string) => setForm((f) => ({ ...f, [k]: v }))
  ...
  return (
    <SettingsForm
      ...
      fields={FIELDS.map((field) => ({
        ...
        control: <Input id={field.k} type={field.type ?? 'text'} placeholder={field.ph} value={form[field.k]} onChange={(e) => set(field.k, e.target.value)} />,
      }))}
      actions={
        <>
