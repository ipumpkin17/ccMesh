...
import { SettingsCenteredAction, SettingsDataTable, SettingsMessage, SettingsStack, SettingsTableAction, SettingsTableActions, SettingsToolbar } from '@/components/settings'
  ...
  return (
    <SettingsStack>
      <SettingsToolbar
        ...
        actions={
          <>
            <Button size="sm" variant="outline" onClick={() => backups.refetch()} disabled={backups.isFetching}>
      ...
      {backups.isError ? (
        ...
        <SettingsDataTable
          columns={[{ label: '文件' }, { label: '大小', align: 'right' }, { label: '时间' }, { label: '操作', align: 'right' }]}
          rows={visibleList.map((backupFile) => [
            ...
            <SettingsTableActions>
              <SettingsTableAction onClick={() => restore.mutate(backupFile.filename)} disabled={restore.isPending}>
                恢复
              </SettingsTableAction>
              <SettingsTableAction onClick={() => del.mutate(backupFile.filename)} disabled={del.isPending}>
                删除
              </SettingsTableAction>
            </SettingsTableActions>,
          ])}
