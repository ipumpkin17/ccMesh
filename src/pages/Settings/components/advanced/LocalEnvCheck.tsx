...
import { AlertCircleIcon, ArrowUpCircleIcon, CheckCircle2Icon, ChevronDownIcon, CopyIcon, DownloadIcon, Loader2Icon, RefreshCwIcon, StethoscopeIcon } from 'lucide-react'
...
const TOOL_ICON_BOX = 'flex size-7 shrink-0 items-center justify-center overflow-hidden rounded-md bg-surface-raised'
const TOOL_ICON_INNER = 'size-[18px] shrink-0'

const TOOL_LOBE_ICONS: Record<Exclude<ToolName, 'pi'>, { Icon: ComponentType<ToolIconProps>; mono?: boolean }> = {
  ...
  if (toolName === 'pi') {
    return (
      <span className={TOOL_ICON_BOX}>
        <img src={piLogoUrl} alt="" className={cn(TOOL_ICON_INNER, 'rounded-[3px] object-cover')} />
      </span>
    )
  }
...
export function LocalEnvCheck() {
  const [toolVersions, setToolVersions] = useState<ToolVersion[]>(() => toolVersionsCache?.data ?? [])
  const [isLoadingTools, setIsLoadingTools] = useState(() => toolVersionsCache === null)
  ...
  const toolVersionByName = useMemo(() => new Map(toolVersions.map((t) => [t.name, t])), [toolVersions])
  ...
  const loadAllToolVersions = useCallback(
    async (options?: { force?: boolean }) => {
      ...
      if (!force && toolVersionsCache && Date.now() - toolVersionsCache.at < CACHE_TTL_MS) {
      ...
      for (const toolName of toolNames) {
        ...
        try {
          ...
          if (tool?.version) {
            ...
            if (action === 'update' && previousVersion && tool.version === previousVersion && isUpdateAvailable(tool.version, latest)) {
              failures.push({
                toolName,
  ...
  const handleRunToolAction = useCallback(
    async (toolNames: ToolName[], action: ToolLifecycleAction) => {
      if (toolNames.length === 0) return
      if (toolNames.some((n) => preflightTools.has(n) || toolActions[n] !== undefined)) {
        return
      }
    ...
    <section className="flex flex-col gap-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Button size="sm" variant="outline" onClick={() => void handleDiagnoseAll()} disabled={isLoadingTools || isAnyBusy || isDiagnosingAll}>
            {isDiagnosingAll ? <Loader2Icon className="size-3.5 animate-spin" /> : <StethoscopeIcon className="size-3.5" />}
            诊断安装冲突
          </Button>
          <Button size="sm" variant="outline" onClick={() => void loadAllToolVersions({ force: true })} disabled={isLoadingTools || isAnyBusy}>
            <RefreshCwIcon className={isLoadingTools ? 'size-3.5 animate-spin' : 'size-3.5'} />
            刷新
          </Button>
          <Button size="sm" onClick={() => void handleRunToolAction(updatableToolNames, 'update')} disabled={isLoadingTools || isAnyBusy || updatableToolNames.length === 0}>
            {batchAction === 'update' ? <Loader2Icon className="size-3.5 animate-spin" /> : <ArrowUpCircleIcon className="size-3.5" />}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {TOOL_NAMES.map((toolName) => {
          ...
          const isToolLoading = Boolean(loadingTools[toolName]) || (isLoadingTools && !toolVersionByName.has(toolName))
          ...
          const action: ToolLifecycleAction | null = isToolLoading || installedButBroken ? null : !tool?.version ? 'install' : isOutdated ? 'update' : null
            ...
            <div key={toolName} className="border-edge-subtle bg-surface-card flex min-h-[150px] flex-col gap-3 rounded-lg border p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <div className="truncate text-sm font-medium">{TOOL_DISPLAY[toolName]}</div>
                    {tool?.env_type && ENV_LABEL[tool.env_type] ? (
                      <span className="border-edge bg-surface-raised text-ink-secondary mt-1 inline-flex rounded-full border px-1.5 py-0.5 text-[9px]">
                ...
                {isToolLoading ? (
                  <Loader2Icon className="text-ink-mute size-4 animate-spin" />
                ) : tool?.version ? (
                  isOutdated ? (
                    <span className="border-warning/20 bg-warning/10 text-warning rounded-full border px-1.5 py-0.5 text-[10px]">可升级</span>
                  ) : (
              ...
              <div className="space-y-1.5 text-xs">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-ink-primary truncate font-mono tabular-nums">
                    {isToolLoading ? '加载中…' : tool?.version ? tool.version : installedButBroken ? '已安装·无法运行' : '未安装'}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-3">
                  <span className="text-ink-secondary">最新版本</span>
                  <span className="text-ink-primary truncate font-mono tabular-nums">{isToolLoading ? '加载中…' : (tool?.latest_version ?? '未知')}</span>
                </div>
                {!isToolLoading && !tool?.version && tool?.error ? <div className="text-ink-mute truncate text-[11px]">{tool.error}</div> : null}
              </div>

              {conflicts && conflicts.length > 0 ? (
                <ul className="border-warning/20 bg-warning/5 space-y-1 rounded-md border p-2">
                  {conflicts.map((inst) => (
                    <li key={inst.path}>
      ...
      <div>
        <button type="button" onClick={() => setShowManual((v) => !v)} className="text-ink-secondary hover:text-ink-primary flex items-center gap-1.5 text-sm">
          <ChevronDownIcon className={`size-3.5 transition-transform ${showManual ? '' : '-rotate-90'}`} />
          手动安装命令
        </button>
        {showManual ? (
