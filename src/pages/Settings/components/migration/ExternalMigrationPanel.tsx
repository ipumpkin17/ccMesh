...
const SOURCES: SourceItemConfig[] = [
  {
    ...
    errorText: (message, hasCustomPath) =>
      hasCustomPath
        ? `读取失败：${message}。请确认所选文件是有效的 CC Switch 数据库。`
        : `自动探测失败：${message}。可尝试「选择文件」。`,
    categoryFilters: [
      {
        id: "claude",
        label: "Claude",
  ...
  },
  {
    ...
    title: "CLI Proxy API",
    description:
      "自动探测本机 CLI Proxy API 配置，或手动选择 cliproxyapi.conf",
    dialogTitle: "CLI Proxy API 配置迁移",
    queryKey: "external-migration-cpa",
    api: cpaSourceApi,
    defaultLoadingText: "正在自动探测 CLI Proxy API 配置…",
    emptyText:
      "未在 CLI Proxy API 配置中找到 openai-compatibility / codex-api-key / claude-api-key 上游凭证。",
    errorText: (message, hasCustomPath) =>
      hasCustomPath
        ? `读取失败：${message}。请确认所选文件是有效的 CLI Proxy API YAML 配置。`
        : `自动探测失败：${message}。可尝试「选择文件」。`,
    categoryFilters: [
      {
        id: "openai-compat",
    ...
    fileFilter: [
      { name: "CLI Proxy API 配置", extensions: ["yaml", "yml", "conf"] },
    ],
  },
];
    ...
    <>
            ...
            control={
              <div className="flex items-center gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => openWithPickedFile(source)}
                >
                  选择文件
                </Button>
                <Button
                  size="sm"
                  onClick={() => openWithAutoDetect(source.id)}
                >
                  自动探测
                </Button>
              </div>
      ...
      {activeSource ? (
        <MigrationImportDialog
          ...
          loadingText={
            activePath
              ? `正在读取所选 ${activeSource.title} 配置…`
              : activeSource.defaultLoadingText
          }
          errorText={(message) =>
            activeSource.errorText(message, Boolean(activePath))
          }
