...
import { NAV_PAGE_IDS, useLayoutStore, type NavPageId } from '@/stores'
      ...
      {NAV_ITEMS.map((item) => {
        ...
        return (
          <SettingsRow
            key={id}
            title={item.label}
            control={<Switch checked={visible} disabled={disableHide} onCheckedChange={(v) => setNavPageVisible(id, v)} aria-label={`显示${item.label}`} />}
          />
        )
      })}
    </SettingsSection>
