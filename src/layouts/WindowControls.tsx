...
export function WindowControls() {
  ...
  return (
    <div className="flex items-center">
      <button type="button" aria-label="最小化" className={btn} onClick={() => appWindow.minimize()}>
        <MinusIcon className="size-3.5" />
      </button>
      <button type="button" aria-label={maximized ? '还原' : '最大化'} className={btn} onClick={() => appWindow.toggleMaximize()}>
        {maximized ? <CopyIcon className="size-3.5" /> : <SquareIcon className="size-3" />}
      </button>
      <button type="button" aria-label="关闭" className={cn(btn, 'hover:bg-destructive hover:text-white')} onClick={() => appWindow.close()}>
