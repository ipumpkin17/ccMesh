...
本文说明仓库 `.github/workflows/` 下两个工作流的职责，以及发版时的产物汇总方式。
...
| 文件          | 触发时机                   | 职责                                                           |
| ------------- | -------------------------- | -------------------------------------------------------------- |
| `ci.yml`      | 手动                       | 三平台编译检查，可用于发版前人工验证                           |
| `release.yml` | push `v*.*.*-*` tag / 手动 | 三平台打包为 Actions Artifact，全部成功后统一创建 Release 草稿 |

核心原则：**正式发版只靠 tag 触发 `release.yml`。** 三端产物全部构建并校验成功后，才会创建 GitHub Release。

---

## 二、缓存如何提速

GitHub Actions 的缓存**按 ref（分支/tag）隔离**，恢复时通常会查找：
...
- 手动在 `master` 跑一次 `ci.yml`，可写入默认分支缓存，后续 tag 构建有机会恢复依赖。
...
- 在 `ci.yml`：手动选择 master 运行时 ref 是 `refs/heads/master` → 条件成立 → **写缓存**。
...
1. 确认当前在 `master` 且工作区干净，运行 `pnpm release`，按提示输入版本号。
...
1. **缓存未命中时 Release 会更慢，但仍能独立完成。** 不要把手动 CI 当作发版前置条件。

2. **升级 Rust 版本时，两个文件的 `toolchain: '1.96.0'` 必须同步改。** 只改一个会导致缓存 key 不一致、命中失效，发版又退回冷编译。

3. **改 `Cargo.lock`（增删依赖）会使缓存部分失效。** 这是正常的；Release 会自行完成缺失编译，需要预热时再手动运行 `ci.yml`。
