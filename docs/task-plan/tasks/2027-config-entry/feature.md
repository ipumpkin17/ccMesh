...
## 附录 A：开关项映射表（**待产品确认**，2029.12 实现前必须定稿）
> 开关以声明式定义驱动：`{ id, label, appType, op:"setEnv"|"setTomlKey"|"mergeCommon", key, onValue, offBehavior:"remove"|"setFalse"|valueWhenOff }`。
> 下表是基于截图与常见用法的**推测默认**，请逐条确认/修正/补全（截图右侧有被裁切的开关）。

### Claude（settings.json，多在 `env` 下）
| 开关 | 推测映射（待确认） |
|---|---|
| 隐藏 AI 署名 | settings 顶层 `includeCoAuthoredBy=false`（或某 env，**待确认**） |
| Teammates 模式 | **待确认**（键名+值） |
| 启用 Tool Search | `env.<?>`（**待确认**键名，开=true/1） |
| 最大强度思考 | `env.MAX_THINKING_TOKENS=<?>`（**待确认**数值） |
| （截图被裁切的其它开关） | **待补充** |

### Codex（config.toml）
| 开关 | 推测映射（待确认） |
|---|---|
| 启用 Goal mode | `<?> = true`（**待确认**键名/所属表） |
| 启用远程压缩 | `<?> = true`（**待确认**） |
| 写入通用配置 | 把"通用配置片段"合并进 config.toml（最小实现；**待确认**片段内容/来源） |
